...
def remove_batch_effect(concat_h5ad,method,cluster,run_tsne,n_trees):
    """
    ...
    if method == 'bbknn':
        ...
        sc.external.pp.bbknn(concat_h5ad,batch_key='batch',n_trees=n_trees)
        sc.tl.umap(concat_h5ad)
        if cluster == 'louvain':
            sc.tl.louvain(concat_h5ad)
        elif cluster == 'leiden':
            sc.tl.leiden(concat_h5ad)
        ...
        if cluster == 'louvain':
            sc.tl.louvain(concat_ref)
        elif cluster == 'leiden':
            sc.tl.leiden(concat_ref)
        sc.tl.umap(concat_ref)
        sc.pl.umap(concat_ref, color=cluster,save='_ingest_ref.png')
        # if run_tsne:
        #     sc.tl.tsne(concat_ref)
            # print('Whether X_tsne in concat_ref.obsm {}'.format('X_tsne' in concat_ref.obsm))
...
def output_data(concat_h5ad,prefix,cluster,outdir):
    ...
    for key in rank_gene_dict:
        ...
        down_df.sort_values(by='avg_logFC',ascending=True,inplace=True)
...
@profile  
def integrate_pipeline(h5ad_dict,method,cluster,outdir,prefix,diff_method,gene_ids_df,run_tsne,n_trees):
    """
    """
    ...
    concat_h5ad = remove_batch_effect(concat_h5ad,method,cluster,run_tsne,n_trees)
    # add gene_ids
...
def main(args):
    ...
    n_trees = args.get('n_trees')
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    integrate_pipeline(h5ad_dict,method,cluster,outdir,prefix,diff_method,gene_ids_df,run_tsne,n_trees)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_tsne','-u',action='store_true',help='run tsne at the same time')
    parser.add_argument('--n_trees','-n',default=10,type=int,help='For bbknn method,The number of trees to construct in the annoy forest. More trees give higher precision when querying, at the cost of increased run time and resource intensity.')
