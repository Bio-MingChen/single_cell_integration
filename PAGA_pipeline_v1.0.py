...
def main(args):
    ...
    if clusters:
        ...
        h5ad = h5ad[h5ad.obs['louvain'].isin(clusters),:]
    ## TODO subset by barcodes
    ...
    if args.get('re_analysis'):
        sc.tl.pca(h5ad)
        sc.pp.neighbors(h5ad, n_pcs=20)
        sc.tl.umap(h5ad)
        sc.tl.louvain(h5ad)
        sc.tl.diffmap(h5ad,n_comps=20)
    else:
    ...
    if not start_cluster:
        start_cluster = h5ad.obs.louvain.cat.categories[0]
    ...
    h5ad.uns['iroot'] = np.flatnonzero(h5ad.obs['louvain'].isin(start_cluster.split(',')))[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c','--clusters',help='indicating which clusters to use')
    parser.add_argument('-b','--barcodes',help='indicating which cells to use')
